...
class AudioTranscriberApp:
    ...
    def browse_file(self):
        ...
        if file_path:
            ...
            self.status_text.set(f"Arxiu seleccionat: {os.path.basename(file_path)} - Idioma: {current_language}")
        ...
        if file_path:
            try:
                with open(file_path, 'w', encoding='utf-8') as file:
                   current_language = self.language_combo.get()
                   file.write(f"Transcripció d'e 'audio - Idioma: {current_language}\n")
